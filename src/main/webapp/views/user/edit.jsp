...
<form:form action="${requestURI }" modelAttribute="userForm">
	...
	<jstl:if test="${userForm.getId()!=0 }">
		<acme:textbox code="actor.username" readonly="readonly"
			path="username" />
	</jstl:if>
	...
	<jstl:if test="${userForm.getId()==0 }">
		<div class="form-check">
			<form:checkbox class="form-check-input" path="check"
				onclick="activar(this.form)" id="check" checked='' />
			<form:label path="check">
				<spring:message code="actor.terminos1" />
			</form:label>
			<a href="termCondition/display.do"><spring:message
					code="actor.terminos2" /></a>
